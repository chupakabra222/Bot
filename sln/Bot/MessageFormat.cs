			...
			try
			{
				...
				if (message != currentMessage)
				{
					...
					foreach (DiscordMessage msg in messages)
					{
						...
						var date2 = msg.CreationTimestamp;
						var dateDiff = date1 - date2;
						if (dateDiff.TotalDays > 13)
							await msg.DeleteAsync();
						else
							messageStack.Push(msg);

						//if (DateTimeOffset.Now.ToUnixTimeSeconds() - msg.CreationTimestamp.ToUnixTimeSeconds() >
						//    DateTimeOffset.Now.AddDays(13).ToUnixTimeSeconds())
						//	await msg.DeleteAsync();
						//else
						//	messageStack.Push(msg);
					}
